...
export default async function Page() {
	...
	// const { latestInvoices, timeAtFetch } = await fetchLatestInvoices();
	// const { totalPaidInvoices, totalPendingInvoices, numberOfInvoices, numberOfCustomers } =
	// 	await fetchCardData();
    const [
		revenue,
		{ latestInvoices, timeAtFetch },
	...
	return (
		<main>
			...
			<div className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-4 lg:grid-cols-8">
				...
				<LatestInvoices latestInvoices={latestInvoices} timeAtFetch={timeAtFetch} />
			</div>
		</main>
	);
}
